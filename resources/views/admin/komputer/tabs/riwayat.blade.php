    ...
    <table class="table table-sm small table-striped" id="historyTable">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>
                    @if (json_decode($barang->serial))
                        CPU: {{ json_decode($barang->serial)->cpu }}<br>
                        Monitor: {{ json_decode($barang->serial)->monitor }}
                    @else
                        -
                    @endif
                </td>
                ...
                <td><div class="progress" style="height: 12px; width: 100px;">
                        <div 
                            class="progress-bar 
                                {{ 
                                    $barang->riwayat->first()->kelayakan >= 75 ? 'bg-success' :
                                    ($barang->riwayat->first()->kelayakan >= 50 ? 'bg-warning' : 'bg-danger')
                                }}"
                            role="progressbar" 
                            aria-valuenow="{{ $barang->riwayat->first()->kelayakan ?? 0 }}" 
                            aria-valuemin="0" 
                            aria-valuemax="100"
                            style="width: {{ $barang->riwayat->first()->kelayakan ?? 0 }}%">
                            {{ $barang->riwayat->first()->kelayakan ?? '-' }}%
                        </div>
                    </div>
                </td>
                                ...
                                <td>
                                    <div class="progress" style="height: 12px; width: 100px;">
                                        <div 
                                            class="progress-bar 
                                                {{ 
                                                    $history->kelayakan >= 75 ? 'bg-success' :
                                                    ($history->kelayakan >= 50 ? 'bg-warning' : 'bg-danger')
                                                }}"
                                            role="progressbar" 
                                            aria-valuenow="{{ $history->kelayakan ?? 0 }}" 
                                            aria-valuemin="0" 
                                            aria-valuemax="100"
                                            style="width: {{ $history->kelayakan ?? 0 }}%">
                                            {{ $history->kelayakan ?? '-' }}%
                                        </div>
                                    </div>
                                </td>
